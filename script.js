...
async function getData() {
    ...
    console.log(`lot: ${lot}`);
    // console.log(`y: ${y}`)
    //https://api.aladhan.com/v1/timings/${y}?latitude=${lat}&longitude=${lot}&method=4 old one
    //https://api.aladhan.com/v1/timings/$%7By%7D?latitude=${lat}&longitude=${lot}&method=4 //new one
    
    let data = await fetch(`https://api.aladhan.com/v1/timings/$%7By%7D?latitude=${lat}&longitude=${lot}&method=4`);
    data = await data.json();
    
    let dar = data.data.date.hijri.month;

    //setting the dateAr and Eng and day
    currentDateTimeAssign[0] = `${Number(dar.number)} ${dar.ar}`; // 2 صفر
    ...
    //city assign 
    //let data2 = await fetch(`https://api.weatherbit.io/v2.0/current?latitiude=${lat}&lon=${lot}&key=61c2db3c31ba4770866bf0ca7f7764f0&include=minutely`);
